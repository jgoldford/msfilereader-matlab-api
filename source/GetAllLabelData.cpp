...
myScan scan;
    ...
    //populate structure array
        ...
        scan.resolution = (float*)malloc((scan.reads)*sizeof(float));
        scan.baseline = (float*)malloc((scan.reads)*sizeof(float));
        scan.charge = (int*)malloc((scan.reads)*sizeof(int));
    ...
    for(read_i=0; (read_i) < sz ;(read_i)++){
        ...
        scan.resolution[(read_i)]= (float)pdval[(read_i)*6+2];
        scan.baseline[(read_i)]= (float)pdval[(read_i)*6+3];
        scan.charge[(read_i)]= (int)pdval[(read_i)*6+5];
...
/*// transfer scan structure to mexStructure array
   ...
   mxArray *reads_out,*intensity_out,*mass_out,*noise_out,*resolution_out,*baseline_out,*charge_out;
   double *reads,*intensity,*mass,*noise,*resolution,*baseline,*charge;
   ...
   resolution_out = plhs[4]= mxCreateDoubleMatrix(sz,1,mxREAL);
   baseline_out = plhs[5]= mxCreateDoubleMatrix(sz,1,mxREAL);
   charge_out = plhs[6]= mxCreateDoubleMatrix(sz,1,mxREAL);
   ...
   resolution = mxGetPr(resolution_out);
   baseline = mxGetPr(baseline_out);
   charge = mxGetPr(charge_out);
   ...
   for(read_i=0; (read_i) < sz ;(read_i)++){
        ...
        resolution[read_i] =  (double)scan.resolution[read_i];
        baseline[read_i] =  (double)scan.baseline[read_i];
        charge[read_i] =  (double)scan.charge[read_i];        
   }
